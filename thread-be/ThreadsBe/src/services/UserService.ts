...
class UserService {
  ...
  async find(req: Request, res: Response) {
    ...
    try {
      ...
      for (const user of followedUsers) {
        const is_follow = user.followersList.some(
          (follower) => follower?.follower?.id === idFollower
        );
  ...
  async getRandomUsers(req: Request, res: Response) {
    try {
      ...
      shuffledUsers.forEach((element) => {
        const isFollowed = follow.some((x) => x.followed.id === element.id);
  ...
  async findUsersByUsernameOrFullname(req: Request, res: Response) {
    try {
      ...
      const matchingUsers = await this.UserRepository.createQueryBuilder("user")
        .where("user.username ILIKE :query OR user.fullname ILIKE :query", {
          query: `%${searchQuery}%`,
        })
        .getMany();
  ...
  //     return res
  //       .status(200)
  //       .json(`data berhasil di ubah: ${JSON.stringify(updatedUser)}`);
  //   } catch (error) {
  ...
  async updateUser(req: Request, res: Response) {
    try {
      ...
      if (data.picture !== "") {
        ...
        // Queue the image update to Cloudinary here
      ...
      return res
        .status(200)
        .json(`Data successfully updated: ${JSON.stringify(updatedUser)}`);
    } catch (error) {
      return res.status(500).json("An error occurred on the server");
    }
  }
