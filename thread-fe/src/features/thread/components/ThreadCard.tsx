...
export function ThreadCard(props: ThreadCard) {
  const [isLiked, setIsLiked] = useState(props.is_like || false);
        ...
        <div style={{ display: "flex" }}>
          ...
          <div>
            ...
            <div>
              ...
              >
                <FaHeart style={isLiked ? { color: "red" } : { color: "grey" }} />
                <h4 style={{ paddingLeft: "20px" }}>{likesCount}</h4>
              </Button>

              <Button background={"white"}>
                <FaCommentDots /> <h4 style={{ paddingLeft: "20px" }}>{props.replies_count}</h4>
              </Button>
            </div>
          </div>
        </div>
